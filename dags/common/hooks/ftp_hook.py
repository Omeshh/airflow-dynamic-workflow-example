    ...
    def storbinary(self, cmd, fp, blocksize=8192, callback=None, rest=None):
        ...
        with self.transfercmd(cmd, rest) as conn:
            ...
            if isinstance(conn, ssl.SSLSocket):
                # Instead of attempting unwrap(Fails for corecard) the connection, just closing the socket.
                conn._real_close()
...
class FTPSHook(FTPHook):
    ...
    def get_conn(self):
        ...
        if self.conn is None:
            ...
            self.conn = ImplicitFTP_TLS()
            self.conn.connect(host=params.host, port=params.port)
            self.conn.login(params.login, params.password)
            self.conn.prot_p()  # Securing the data connection
            self.conn.set_pasv(pasv)
